...
def demo_gan():
    ...
    st.markdown("---")
    st.markdown("Creator: [Priyanka Sukumar](https://priyankasukumar22.github.io)")
    st.markdown(
        "Code: [GitHub Repository](https://github.com/PriyankaSukumar22/T2I)"
    )
    st.markdown("---")
    ...
    if user_input:
        ...
        st.write(f"Total time for training:")
        st.write(total_t)
...
def demo_coco():
    ...
    if user_input:
        ...
        st.text("")
        center_element(
            type="image", img_path="models/coco_AttnGAN2/output/0_s_0_g2.png"
        )
        center_element(type="subheading", text="The attention given for each word")
        st.image("models/coco_AttnGAN2/output/0_s_0_a1.png")

        st.markdown("---")
        with st.expander("Click to see the first stage images"):
            st.write("First stage image")
            st.image("models/coco_AttnGAN2/output/0_s_0_g1.png")
            st.write("First stage attention")
            st.image("models/coco_AttnGAN2/output/0_s_0_a0.png")
...
def attngan_explained():
    ...
    st.header(
        "**T2I**:  Text To Image Synthesis using Attentional Generative Adverserial Networks"
    )
